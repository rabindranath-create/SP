    ...
    while(is.null(output)){
      ...
      for(k in 1:length(V(G_new))){
        ...
        Q_lambda_b <- get.shortest.paths(G_new,V(G_new)[k],
                                         ...
                                         output="both",
                                         weights=df_edge$Gamma_lambda,algorithm="dijkstra")
        if(length(Q_lambda_f$vpath[[1]])==0|length(Q_lambda_b$vpath[[1]])==0){
          C_lambda_fb[k] <- Inf
          W_lambda_fb[k] <- W
        } else{
...
# SR risk function - cost as alpha
Update_graph_intersect<-function(g,x,y,circle_info,r){
  ...
  for(i in 1:n){
    ...
    for(j in 1:n1){
      ...
      elg[index,3] <- elg[index,3]-0.5*circle_info[i,3]*log(1-circle_info[i,4])
        ...
        if (length(obs_ind_temp)==1){
          ...
          if (obs_info$status[obs_ind_temp]==1){
            ...
            df_edge_ed[which(Int_info[,obs_ind_temp]==1),3] <- df_edge_ed[which(Int_info[,obs_ind_temp]==1),3]+
              0.5*obs_info[obs_ind_temp,3]*log(1-obs_info[obs_ind_temp,4])
          ...
          if(obs_info$status[obs_ind_temp2]==1){
            ...
            df_edge_ed[which(Int_info[,obs_ind_temp2]==1),3] <- df_edge_ed[which(Int_info[,obs_ind_temp2]==1),3]+
              0.5*obs_info[obs_ind_temp,3]*log(1-obs_info[obs_ind_temp2,4])
...
for (i in 1:100){
  ...
  result <- WCSPP_Node_risk_C(obs_info_all[[i]])
    ...
    result_WCSPP_risk_C[i ,1] <- result[[i]]$Length_total
    result_WCSPP_risk_C[i ,2] <- result[[i]]$Cost_total
    result_WCSPP_risk_C[i ,3] <- length(result[[i]]$Disambiguate_state)
    result_WCSPP_risk_C[i ,4] <- result[[i]]$LU_diff[1]
    result_WCSPP_risk_C[i ,5] <- result[[i]]$LU_diff[2]
    result_WCSPP_risk_C[i ,6] <- result[[i]]$LU_diff[3]
    result_WCSPP_risk_C[i ,7] <- result[[i]]$LU_diff[4]
