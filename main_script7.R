...
WCSPP_Node_DT <- function(obs_info){
  W <- 4
  x <- 100; y <- 50; r <- 5
  # create graph
...
write.csv(result_WCSPP_DT, file = file.path(output_dir, "result_WCSPP_DT_20_4.csv"))
for (i in 1:10){
  obs_info_all_use <- obs_info_all[(10*(i-1)+1):(10*i)]
  result <- parLapply(cl,obs_info_all_use,WCSPP_Node_DT)
  for (j in 1:10){
    result_WCSPP_DT[10*(i-1)+j,1] <- result[[j]]$Length_total
    result_WCSPP_DT[10*(i-1)+j,2] <- result[[j]]$Cost_total
    result_WCSPP_DT[10*(i-1)+j,3] <- length(result[[j]]$Disambiguate_state)
    result_WCSPP_DT[10*(i-1)+j,4] <- result[[j]]$LU_diff[1]
...
result_WCSPP_DT <- matrix(NA,ncol=7,nrow=100)
write.csv(result_WCSPP_DT, file = file.path(output_dir, "result_WCSPP_DT_20_6.csv"))
...
result_WCSPP_DT <- matrix(NA,ncol=7,nrow=100)
write.csv(result_WCSPP_DT, file = file.path(output_dir, "result_WCSPP_DT_40_6.csv"))
...
result_WCSPP_DT <- matrix(NA,ncol=7,nrow=100)
write.csv(result_WCSPP_DT, file = file.path(output_dir, "result_WCSPP_DT_40_8.csv"))
...
result_WCSPP_DT <- matrix(NA,ncol=7,nrow=100)
write.csv(result_WCSPP_DT, file = file.path(output_dir, "result_WCSPP_DT_80_8.csv"))
...
result_WCSPP_DT <- matrix(NA,ncol=7,nrow=100)
write.csv(result_WCSPP_DT, file = file.path(output_dir, "result_WCSPP_DT_80_10.csv"))
