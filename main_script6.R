        ...
        if(round(L_new,5)==round(phi_lambda,5)){
          ...
          if(is.null(index_delete)){
            output <- list(Optimal_upper=T, Optimal_multiplier=T, Value_upper=Upper, 
                           Info_path=P_upper, Lambda_pos=lambda_pos, Lambda_neg=lambda_neg,
                           Lambda_new=lambda_new, Graph=G_update,Step=6, Value_lower=min(phi_lambda_fb))
          } else{
            output <- list(Optimal_upper=T, Optimal_multiplier=T, Value_upper=Upper, 
                           Info_path=P_upper, Lambda_pos=lambda_pos, Lambda_neg=lambda_neg,
...
#obs_info_all1 <- read.csv('obs_info_all.csv')
...
write.csv(result_WCSPP_RD, file = file.path(output_dir, "result_WCSPP_RD_20_4.csv"))
...
for (i in 1:10){
  ...
  result <- parLapply(cl,obs_info_all_use,WCSPP_Node_RD)
  for (j in 1:10){
    result_WCSPP_RD[10*(i-1)+j,1] <- result[[j]]$Length_total
    result_WCSPP_RD[10*(i-1)+j,2] <- result[[j]]$Cost_total
    result_WCSPP_RD[10*(i-1)+j,3] <- length(result[[j]]$Disambiguate_state)
    result_WCSPP_RD[10*(i-1)+j,4] <- result[[j]]$LU_diff[1]
...
####  RD risk function ####
result_WCSPP_RD <- matrix(NA,ncol=7,nrow=100)
write.csv(result_WCSPP_RD, file = file.path(output_dir, "result_WCSPP_RD_20_6.csv"))
...
####  RD risk function ####
result_WCSPP_RD <- matrix(NA,ncol=7,nrow=100)
write.csv(result_WCSPP_RD, file = file.path(output_dir, "result_WCSPP_RD_40_6.csv"))
...
####  RD risk function ####
result_WCSPP_RD <- matrix(NA,ncol=7,nrow=100)
write.csv(result_WCSPP_RD, file = file.path(output_dir, "result_WCSPP_RD_40_8.csv"))
Update_graph_intersect<-function(g,x,y,circle_info,r){
  #read circle center x,y coordinate c-cost,p-prabability,True or False Obstacles
  #circles=read.csv("example1.csv",header=FALSE)
  n <- nrow(circle_info)
...
####  RD risk function ####
result_WCSPP_RD <- matrix(NA,ncol=7,nrow=100)
write.csv(result_WCSPP_RD, file = file.path(output_dir, "result_WCSPP_RD_80_8.csv"))
...
####  RD risk function ####
result_WCSPP_RD <- matrix(NA,ncol=7,nrow=100)
write.csv(result_WCSPP_RD, file = file.path(output_dir, "result_WCSPP_RD_80_10.csv"))
